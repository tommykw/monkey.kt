...
val definition: Map<Opcode, Definition> = mapOf(
    ...
    OpGreaterThan to "OpGraterThan".toDefinition(),
    ...
    OpGetBuiltin to "OpGetBuiltin".toDefinition(1),
...
private fun String.toDefinition(vararg operandsWidths: Int) =
    Definition(this, intArrayOf(*operandsWidths))
...
fun readOperands(def: Definition, ins: Instructions): Pair<IntArray, Int> {
    ...
    val operands = def.operandsWidths.map { width ->
        when (width) {
            1 -> ins.readInt(offset)
            2 -> ins.offset(offset).readByte().toInt()
            else -> width
        }.also { offset += width }
    }
...
fun lookup(op: Opcode): Definition {
    return definition.getOrElse(op) {
        throw IllegalArgumentException("opcode $op undefined")
    }
}
